  ...
  <div class="experiment">
    <form class="radial-lines-settings-form heading">
      ...
      <figure class="line-size-group">
        <label for="numLines">
          <input type="number" id="numLines" placeholder="6" value="6" v-on:keyup.enter="submitProps">Number of lines
        </label>
        <label for="startColor">
          <input type="color" value="#00aaff" id="startColor" v-model="startColor" v-on:keyup.enter="submitProps">Starting Color (hex)
        </label>
        <label for="radiusStart">
          <input type="number" id="radiusStart" placeholder="50" value="50" v-on:keyup.enter="submitProps">Starting Radius
        </label>
        <label for="lineLength">
          <input type="number" id="lineLength" placeholder="100" value="100" v-on:keyup.enter="submitProps">Line Length
        </label>
        <label for="strokeWidth">
          <input type="number" id="strokeWidth" placeholder="15" value="15" v-on:keyup.enter="submitProps">Line Width
        </label>
      </figure>

      <figure class="hsla-group">
        <label for="hue">
          <input type="number" id="hue" placeholder="40" value="40" v-on:keyup.enter="submitProps">Modify Hue
        </label>
        <label for="saturation">
          <input type="number" id="saturation" placeholder="100" value="100" v-on:keyup.enter="submitProps">Modify Saturation
        </label>
        <label for="lightness">
          <input type="number" id="lightness" placeholder="50" value="50" v-on:keyup.enter="submitProps">Modify Lightness
        </label>
        <label for="alpha">
          <input type="number" id="alpha" placeholder="1" value="1" v-on:keyup.enter="submitProps">Modify Opacity
        </label>
      </figure>

      <button class="button" :style="`background: ${startColor}`" @click="submitProps">Apply</button>
    </form>
    <RadialLines :numLines="numLines" :startColor="startColor" :radiusStart="radiusStart" :lineLength="lineLength" :strokeWidth="strokeWidth" :H="H" :S="S" :L="L" :A="A" :renderSvg="renderSvg" />
